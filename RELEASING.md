...
### 1b. Man Pages (optional asset) *(next release)*
...
### 4. Verify Sigstore Signatures *(next release)*
...
### 5. Verify SLSA Provenance *(next release)*
...
build → github-draft ─┬→ pypi-publish ──┐
                      ...
                      ├→ provenance (non-blocking, next release)
                      └→ image (GHCR, non-blocking, next release)
```
...
`id-token: write`, `sigstore/cosign-installer` pinned by SHA) *(next release)*;
...
- **`binaries`**: 3-OS matrix (ubuntu/windows/macos-latest) that runs `scripts/package.py` to produce PyInstaller onefile executables, smoke-tests each binary, Sigstore-signs each artifact *(next release)*, and attaches the binaries + per-OS `SHA256SUMS` + `.sigstore.json` bundles to the draft release. It fans out after `github-draft` and **never gates `pypi-publish`** — PyPI ships only wheel+sdist. See [Native Binary Distribution](#native-binary-distribution).
- **`provenance`**: after `github-draft`, generates SLSA v1 provenance for the canonical artifact set via `slsa-framework/slsa-github-generator` (`generator_generic_slsa3.yml`, pinned by SHA) with `actions: read` + `id-token: write` + `contents: write`; uploads `sesslint-provenance.intoto.jsonl` to the release. Intentionally off the promote critical path until validated end-to-end *(next release)*.
- **`image`**: downloads the ubuntu-leg binary via a 1-day workflow artifact, builds the `Dockerfile` image (distroless nonroot — glibc for the PyInstaller binary, no shell), smoke-tests `version --json`, then pushes `ghcr.io/<owner>/sesslint:<tag>` + `:latest` with `packages: write`. Non-blocking: `github-promote` does not wait on it *(next release)*.
