...
def dump_canonical(
    ...
    """Serialize canonical events and source metadata into deterministic JSON bytes.
    ...
    - Trailing newline appended (exact canonical JSON bytes).
...
def load_canonical(
    ...
    *,
    ...
    try:
        ...
        # Strict JSON decode (also catches trailing garbage after single document)
        try:
        ...
        if raw_version != 1 and norm_ver not in SUPPORTED_CANONICAL_VERSIONS:
            add_finding(
                make_finding(
                    code=SL301,
                    severity=Severity.ERROR,
        ...
        if isinstance(raw_source, dict):
            ...
            add_finding(
                make_finding(
                    ...
                    source=SourceRef(path=path_str, line=1, record_id=None),
                    evidence={"reason": "invalid_source_type", "type": type(raw_source).__name__},
                )
            )
        ...
        if (
            ...
            raise MaxRecordsExceededError(
                f"Record count {len(raw_events)} exceeds limit of {effective_limits.max_records}"
            )
        ...
        for idx, ev_raw in enumerate(raw_events):
                    ...
                    try:
                        ...
                        add_finding(
                            make_finding(
                                ...
                                message_template=_MSG_INVALID_TS,
                                source=SourceRef(
                                    path=path_str, line=line_num, record_id=rec_id_str
                                ),
                                related_ids=("ts",),
                                evidence={
                                    "reason": "invalid_timestamp",
            ...
            for k, v in ev_raw.items():
                if k not in KNOWN_EVENT_KEYS:
                    if (
                        k in CRITICAL_KEYS
                        or k.startswith("critical_")
                        or k.startswith("unknown_critical")
                    ):
                        if not has_emitted_event_sl302:
                            add_finding(
                                make_finding(
                                    ...
                                    repairability=Repairability.MANUAL,
                                    message_template=(
                                        "Unknown critical record on line {line}"
                                        " for record {record_id}"
                                    ),
                                    source=SourceRef(
                                        path=path_str, line=line_num, record_id=rec_id_str
            ...
            content_hash = (
                str(ev_raw["content_hash"]) if ev_raw.get("content_hash") is not None else None
            )
